...
import javax.web.skeleton4j.benchmark.constants.WebEventKey;
 ...
 */
@Data
public class EventKeyDownRequest implements Serializable{
    @ApidocElement(value = "姓名", interfaces = {
            @WebCascadeInterface(serviceClass = EventTestService.class,
                    ...
                    cascadeEvent = WebTriggerEvent.KEY_DOWN,
                    eventKey = WebEventKey.CTRL_Q,
            ...
            @WebCascadeInterface(serviceClass = EventTestService.class,
                    ...
                    params = @WebParam(name = "name", value = "${name}"),
                    cascadeEvent = WebTriggerEvent.KEY_DOWN,
                    eventKey = WebEventKey.CTRL_ALT_Q,
                    resultDisplay = @WebResultDisplay(column = "result1", displayType = WebDisplayType.DATA_SOURCE)
            ),
            @WebCascadeInterface(serviceClass = EventTestService.class,
                    value = "eventTestName",
                    params = @WebParam(name = "name", value = "${name}"),
                    cascadeEvent = WebTriggerEvent.KEY_DOWN,
                    eventKey = WebEventKey.CTRL_ALT_SHIFT_Q,
                    resultDisplay = @WebResultDisplay(column = "result1", displayType = WebDisplayType.DATA_SOURCE)
            ),
    })
    String name;
