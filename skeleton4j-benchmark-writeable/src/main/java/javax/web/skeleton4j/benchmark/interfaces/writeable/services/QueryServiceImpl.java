...
public class QueryServiceImpl implements QueryService {
    ...
    @Override
    public Query20_2Response query20_2(QueryRequest request) {
        Query20_2Response response = new Query20_2Response();
        response.setPageSize(request.getPageSize());
        response.setPageNo(request.getPageNo());
        response.setPageNum(5);
        response.setTotal(5 * request.getPageSize());
        for (int i = 0; i < request.getPageSize(); i++) {
            Record20_2 record = new Record20_2();
            record.setSerialNo(UUID.randomUUID().toString());
            response.getRecords().add(record);
        }
        return response;
    ...
    @Override
    public Query22Response query22(QueryRequest request) {
        Query22Response response = new Query22Response();
        response.setPageSize(request.getPageSize());
        response.setPageNo(request.getPageNo());
        response.setPageNum(5);
        response.setTotal(5 * request.getPageSize());
        for (int i = 0; i < request.getPageSize(); i++) {
            Record22 record = new Record22();
            record.setSerialNo(UUID.randomUUID().toString());
            response.getRecords().add(record);
        }
        return response;
