 ...
 */
@WebPage(displayName = "编辑对话框（下拉列表多选 本地数据源）",
        priority = 5,
        author = "rnkrsoft.com",
        namespaces = {
                @WebNamespace(
                        hints = "分页查询->编辑对话框（下拉列表多选 本地数据源）",
                        imports = {
                                @WebImport(gui = WebGui.FORM, mode = WebMode.QUERY, serviceClass = QueryService.class, value = "query5")
                        })
        }
)
